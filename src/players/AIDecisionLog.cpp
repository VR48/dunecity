...
uint64_t DecisionLog::write(uint32_t cycle, int house, int player,
    ...
    const auto row = Record().set("schema_version", 1).set("telemetry_version", 11).set("policy_version", "transport-tax-hedge-v59")
    ...
    if (bytes + row.size() > limit) {
        ...
        stream << Record().set("schema_version", 1).set("telemetry_version", 11).set("policy_version", "transport-tax-hedge-v59").set("session", session).set("seq", id)
