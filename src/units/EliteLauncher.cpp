...
EliteLauncher::EliteLauncher(InputStream& stream) : TrackedUnit(stream) {
    EliteLauncher::init();
}
...
EliteLauncher::~EliteLauncher() = default;

void EliteLauncher::blitToScreen() {
    ...
    const Coord eliteLauncherTurretOffset[] = {
        Coord(0, -12),
        Coord(0, -8),
        Coord(0, -8),
        Coord(0, -8),
    ...
    SDL_Rect dest2 = calcSpriteDrawingRect(pTurretGraphic,
                                           screenborder->world2screenX(realX + eliteLauncherTurretOffset[drawnAngle].x),
                                           screenborder->world2screenY(realY + eliteLauncherTurretOffset[drawnAngle].y),
...
bool EliteLauncher::canAttack(const ObjectBase* object) const {
    return ((object != nullptr)
            && ((object->getOwner()->getTeamID() != owner->getTeamID()) || object->getItemID() == Unit_Sandworm)
            && object->isVisible(getOwner()->getTeamID()));
}

void EliteLauncher::playAttackSound() {
